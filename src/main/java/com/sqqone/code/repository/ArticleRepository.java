...
public interface ArticleRepository extends JpaRepository<Article,Integer>, JpaSpecificationExecutor<Article> {
    ...
    //10条下载最多资源（热搜推荐）
    @Query(value = "SELECT * FROM  article  join user_download \n" +
            "where article.article_id=user_download.article_id   \n" +
            "GROUP BY user_download.article_id ORDER BY COUNT(user_download.article_id) desc limit ?1",nativeQuery = true)
