...
import com.sqqone.code.service.*;
import com.sqqone.code.util.*;
...
public class UserController {
    ...
    public Map<String,Object> login(User user, HttpSession session){
        ...
        if (StringUtil.isEmpty(user.getUserName())){
            ...
            UsernamePasswordToken usernamePasswordToken = new UsernamePasswordToken(user.getUserName(),CryptographyUtil.md5(user.getPassword(),CryptographyUtil.SALT));
            try {
                subject.login(usernamePasswordToken);//登录验证
                String userName = (String) SecurityUtils.getSubject().getPrincipal();
                User currentUser = userService.findByUserName(userName);
                if (currentUser.isOff){
                    map.put("success",false);
                    map.put("errorInfo","该用户已被封禁！");
                    subject.logout();
                }else{
    ...
    public Map<String,Object> SendEmail(String email, HttpSession session){
        ...
        message.setFrom("2455628530@qq.com");
        message.setTo(email);
        message.setSubject("java sharing web 找回密码");
        message.setText("本次验证码是:"+mailCode);
...
//        用户重置密码
        ...
        user.setPassword(CryptographyUtil.md5(Consts.DefaultPassword,CryptographyUtil.SALT));
        userService.save(user);
        map.put("success", true);
        return map;
    }
    ...
    public Map<String,Object> saveArticle(Article article,HttpSession session) throws IOException {
        ...
        if (!CheckShareLinkEnableUtil.check(article.getDownload())){
            map.put("success",false);
            map.put("errorInfo","该链接已失效！请重新发布");
            return map;
        }
        User currentUser = (User) session.getAttribute(Consts.CURRENT_USER);
        /*判断新增资源还是修改资源*/
        if(article.getArticleId()==null){
            ...
            if(oldArticle.getUser().getUserId().intValue()==currentUser.getUserId().intValue()){        //只能修改自己的资源
                ...
                if(oldArticle.getState().intValue()==3){           //假如原先是未通过，用户点击修改，则重新审核，状态变成未审核
...
//                更新时需要把审核已经通过的新资源信息放入lucene
    ...
    public Map<String,Object> checkArticleUser(Integer articleId,HttpSession session) {
        ...
        if(article.getUser().getUserId().intValue()==currentUser.getUserId().intValue()){
            ...
            map.put("success",false);
            map.put("errorInfo","您不是资源所有者！不能修改该资源");
        }
        return map;
    }
    ...
    public ModelAndView toEditArticle(@PathVariable(value = "articleId",required = true) Integer articleId){
        ...
        mav.addObject("article",articleService.getById(articleId));
    ...
    public Map<String,Object> articleDelete(Integer articleId,HttpSession session) {
        ...
        if(article.getUser().getUserId().intValue()==currentUser.getUserId().intValue()){
            ...
            map.put("success",false);
            map.put("errorInfo","您不是资源所有者！不能删除该资源");
        }
    ...
    public boolean userDownloadExist(Integer articleId,HttpSession session){
        User currentUser = (User) session.getAttribute(Consts.CURRENT_USER);
        Long count = userDownloadService.getCountByUserIdAndArticleId(currentUser.getUserId(), articleId);
        if(count>0){
            return true;
        }else{
            return false;
    ...
    public ModelAndView toHaveDownloaded(@PathVariable(value = "currentPage",required = false)Integer currentPage,
        ...
        User currentUser = (User)session.getAttribute(Consts.CURRENT_USER);
        //已下载资源的列表
        Page<UserDownload> page = userDownloadService.list(currentUser.getUserId(),currentPage,Consts.PAGE_SIZE,
                Sort.Direction.DESC,"downloadDate");
        mav.addObject("userDownloadList",page.getContent());
        //分页html代码
        mav.addObject("pageStr", HTMLUtil.getPagation("/user/toHaveDownloaded",
                page.getTotalPages(),currentPage,"没有下载任何资源。。。"));
    ...
    public Map<String,Object> updateHeadPortrait(MultipartFile file, HttpSession session) throws Exception {
        ...
        if(!file.isEmpty()){
            ...
            FileUtils.copyInputStreamToFile(file.getInputStream(),new File(imgFilepPath+newFileName));
            map.put("success",true);
    ...
    public Map<String,Object> updatePassword(String oldPassword,String newPassword,HttpSession session){
        ...
        if(currentUser.getPassword().equals(CryptographyUtil.md5(oldPassword,CryptographyUtil.SALT))){
            User oldUser = userService.getUserById(currentUser.getUserId());
